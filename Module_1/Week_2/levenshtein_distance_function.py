def del_cost_function(source_char):
    return 1


def ins_cost_function(target_char):
    return 1
...
def levenshtein_function(source, target):
    ...
    for i in range(1, source_len):
        d_matrix[i][0] = d_matrix[i-1][0] + del_cost_function(d_matrix[i][0])

    for i in range(1, target_len):
        d_matrix[0][i] = d_matrix[0][i-1] + ins_cost_function(d_matrix[0][i])

    result = None
    for i in range(1, source_len):
        for j in range(1, target_len):
            del_cost = d_matrix[i-1][j] + del_cost_function(d_matrix[i][j])
            ins_cost = d_matrix[i][j-1] + ins_cost_function(d_matrix[i][j])
